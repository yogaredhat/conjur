require 'spec_helper'

describe "routing for authentication", :type => :routing do
  it "routes POST /authn/the-account/kevin.gilpin@inscitiv.com/authenticate to authenticate#authenticate" do
    expect(post: '/authn/the-account/kevin.gilpin@inscitiv.com/authenticate').to route_to(
      controller: 'authenticate',
      action: 'authenticate',
      account: 'the-account',
      id: 'kevin.gilpin@inscitiv.com'
    )
  end

  it "routes POST /authn-kubernetes/the-service-id/the-account/kevin.gilpin@inscitiv.com/authenticate to kuberneetes/authenticate#authenticate" do
    expect(post: '/authn-kubernetes/the-service-id/the-account/kevin.gilpin@inscitiv.com/authenticate').to route_to(
      controller: 'kubernetes/authenticate',
      action: 'authenticate',
      service_id: 'the-service-id',
      account: 'the-account',
      id: 'kevin.gilpin@inscitiv.com'
    )
  end
    
  it "routes PUT /authn/the-account/password to credentials#update_password" do
    expect(put: '/authn/the-account/password').to route_to(
      controller: 'credentials',
      account: 'the-account',
      action: 'update_password'
    )
  end
end
