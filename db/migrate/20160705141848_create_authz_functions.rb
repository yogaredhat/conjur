...
Sequel.migration do
  up do
    execute %(CREATE OR REPLACE FUNCTION all_roles(role_id text) RETURNS TABLE(role_id text, admin_option boolean)
    ...
    execute %(CREATE OR REPLACE FUNCTION roles_that_can(permission text, resource_id text) RETURNS SETOF roles
  ...
  down do
    execute %(DROP FUNCTION roles_that_can(text, text))
    execute %(DROP FUNCTION is_role_allowed_to(text, text, text))
    execute %(DROP FUNCTION all_roles(text))
  end
end
