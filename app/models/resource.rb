...
class Resource < Sequel::Model
  ...
  def as_json(options = {})
    super(options).tap do |response|
      ...
      response['annotations'] = annotations.as_json.map { |h| h.except 'resource' }
      case kind
      when 'variable'
        response['secrets'] = secrets_dataset.order(:version).as_json
                                             .map { |h| h.except 'resource' }
      when 'policy'
        response['policy_versions'] = policy_versions.as_json.map { |h| h.except 'resource' }
      when 'host_factory'
        response['tokens'] = host_factory_tokens.as_json.map { |h| h.except 'resource' }
        response['layers'] = role.layers.map(&:role_id)
      when 'user', 'host'
        response['restricted_to'] = role.restricted_to.map(&:to_s)
      end
    end
  end
