module AuthnK8sWorld
  ...
  def substitute(pattern)
    ...
    pattern.gsub! '@pod_ip_dashes@', @pod.status.podIP.tr('.', '-') if @pod
  ...
  def pod_certificate
    ...
    if response[:error] && response[:error].join =~ /command terminated with non-zero exit code/
      warn "Unable to retrieve client certificate for pod #{@pod.metadata.name.inspect}"
    else
  ...
  def detect_request_ip(objectid)
    ...
    if pod
      warn "Using Pod #{pod.metadata.name} for objectid #{objectid}"
      pod.status.podIP
    else
      warn "No pod found for objectid #{objectid}"
      '192.0.2.0'
    end
  end
end
